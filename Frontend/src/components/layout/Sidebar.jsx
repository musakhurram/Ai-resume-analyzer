...
const Sidebar = ({ open, onNavigate, onClose }) => {
  ...
  useEffect(() => {
    ...
    const loadBilling = async () => {
      try {
        ...
        // Keep the safe Free/3 fallback when billing is temporarily unavailable.
      }
    };
  ...
  const creditLabel = `${credits} AI generation${credits === 1 ? "" : "s"} remaining`;
  ...
  return <aside className={`sidebar ${open ? "is-open" : ""}`} aria-label="Main sidebar">
    ...
    <div className="sidebar__footer">
      <div className={`sidebar__status-card ${credits === 0 ? "is-empty" : ""}`}>
        ...
        <p className="sidebar__status-meta">{credits > 0 ? "Available for ATS analysis and AI optimization" : planLabel === "Free" ? "Your 3 free generations are used. Upgrade to continue." : "Purchase another plan pack to continue"}</p>
      </div>
    </div>
  </aside>;
};
