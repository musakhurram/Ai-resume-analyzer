...
const navItems = [
  ...
  { to: "/pricing", alias: ["/pricing"], label: "Plans & Billing", icon: <svg viewBox="0 0 20 20" fill="none"><path d="m10 2.8 1.85 4.65 4.95.35-3.78 3.12 1.2 4.78L10 13.05l-4.22 2.65 1.2-.?" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round" /></svg> },
];
...
const Sidebar = ({ open, onNavigate, onClose }) => {
  ...
  useEffect(() => {
    ...
    const loadBilling = async () => {
      try {
        ...
        // Never display a fake balance when the billing endpoint is unavailable.
  ...
  const planLabel = billing?.planLabel || (plan === "premium" ? "Premium" : plan === "pro" ? "Pro" : "Free");
  const hasBalance = Number.isFinite(Number(billing?.aiTokens));
  const tokens = hasBalance ? Math.max(0, Number(billing.aiTokens)) : 0;
  const allowance = Math.max(1, Number(billing?.planTokens) || (plan === "premium" ? 75000 : plan === "pro" ? 25000 : 3000));
  const percentage = hasBalance ? Math.min(100, Math.max(0, (tokens / allowance) * 100)) : 0;
  ...
  return <aside className={`sidebar ${open ? "is-open" : ""}`} aria-label="Main sidebar">
    ...
    <div className="sidebar__footer">
      <div className={`sidebar__status-card ${hasBalance && tokens === 0 ? "is-empty" : ""}`}>
        ...
        {billingLoading ? <p className="sidebar__status-desc">Checking token balance…</p> : <>
          <div className="sidebar__token-row"><p className="sidebar__status-desc">{tokenLabel}</p>{hasBalance && <span className="sidebar__token-percent">{Math.round(percentage)}%</span>}</div>
          {hasBalance && <>
            <div className="sidebar__token-track" role="progressbar" aria-label="AI tokens remaining" aria-valuemin="0" aria-valuemax={allowance} aria-valuenow={tokens}><span className="sidebar__token-fill" style={{ width: `${percentage}%` }} /></div>
            <div className="sidebar__token-meta"><span>{used.toLocaleString()} used</span><span>{allowance.toLocaleString()} total</span></div>
          </>}
          <p className="sidebar__status-meta">{billingError ? "Unable to sync your balance. Refresh to try again." : tokens > 0 ? "Available across your AI features" : planLabel === "Free" ? "Your 3,000 free tokens are used. Upgrade to continue." : "Purchase another token pack to continue"}</p>
        </>}
      </div>
    </div>
  </aside>;
