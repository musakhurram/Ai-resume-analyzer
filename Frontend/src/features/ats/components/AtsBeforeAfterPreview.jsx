...
const Icon = ({ children, size = 20, className = "" }) => <span className={`ats-inline-icon ${className}`} style={{ width: size, height: size, flex: `0 0 ${size}px` }}>{children}</span>;
const SparkleIcon = ({ size = 20 }) => <Icon size={size}><svg viewBox="0 0 24 24" fill="none"><path d="M12 3l1.8 5.2L19 10l-5.2 1.8L12 17l-1.8-5.2L5 10l5.2-1.8L12 3z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round"/><path d="m19 15 .8 2.2L22 18l-2.2.8z" fill="currentColor"/></svg></Icon>;
  ...
  return <div className="ats-preview-modal glass-panel">
    <section className="ats-preview-toolbar" aria-label="AI revision controls">
      ...
      <div className="ats-preview-toolbar__row">
        ...
        <Button variant="primary" size="sm" loading={loading} onClick={handleRunRevise} className="ats-toolbar-btn"><span className="ats-generate-icon" aria-hidden="true"><SparkleIcon size={16}/></span>{hasRevision ? "Re-generate" : "Generate Revision"}</Button>
      </div>
    </section>
    {!hasRevision && !loading && <><section className="ats-preview-hero"><div className="ats-preview-hero__copy"><div className="glow-pill"><span className="glow-pill__dot"/><span>AI REVISION STUDIO</span></div><h2 className="ats-preview-modal__title">PDF-to-PDF comparison</h2><p className="ats-preview-modal__desc">Generate an AI revision to see additions and replacements highlighted at their actual locations inside your resume.</p></div><HeroComparisonVisual/></section>{error && <Callout tone="error" title="Revision Issue">{error}</Callout>}</>}
