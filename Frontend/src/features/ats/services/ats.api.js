...
export async function reviseAtsResume({
  id,
  sections = "all",
  customNotes = "",
}) {
...
export async function saveAtsManualRevision(id, revisedResume) {
  const response = await api.put(`/api/resume/ats-revision/${id}`, {
    revisedResume,
  });
  return response.data;
}
...
export async function getAtsOriginalPdf(id, originalPdfUrl = "") {
  if (originalPdfUrl?.startsWith("blob:")) {
    ...
    if (!response.ok)
      throw new Error(
        `Unable to read the uploaded resume PDF (${response.status}).`,
      );
...
export async function sendAtsResumeByEmail({
  id,
  senderEmail,
  recipient,
  subject,
