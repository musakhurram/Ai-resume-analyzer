  ...
  return (
    <div className="ats-reports-page">
      <header className="page-header">
        ...
        <p className="page-header__desc">Revisit previous ATS audits, compare scores, and open any saved review without running the analysis again.</p>
      </header>
      {error && <Callout tone="error" title="Couldn't load ATS history">{error}</Callout>}
      {stats && <div className="ats-reports-page__stats glass-panel"><div><strong>{stats.count}</strong><span>Reviews</span></div><div><strong>{stats.average}%</strong><span>Average Score</span></div><div><strong>{stats.best}%</strong><span>Best Score</span></div></div>}
      <div className="ats-reports-page__toolbar glass-panel">
        <input className="ats-reports-page__search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search resume name or review title…" aria-label="Search ATS reports" />
        <div className="ats-reports-page__filters" role="tablist" aria-label="ATS report categories">
          {CATEGORIES.map((item) => <button key={item.value} type="button" className={category === item.value ? "is-active" : ""} onClick={() => setCategory(item.value)}>{item.label}</button>)}
        </div>
      </div>
      {!reports.length ? (
        <EmptyState eyebrow="No ATS reviews" title="Your ATS archive is empty" description="Analyze a resume and the review will automatically appear here for your account." action={<Button as={Link} to="/analyze/ats-score" variant="primary">Run ATS Review</Button>} />
      ) : (
