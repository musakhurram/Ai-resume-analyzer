...
const Register = () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message ||
          err.message ||
          "Registration failed. An account may already exist with this email or username.",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  if (loading) {
    return <PageLoader label="Configuring your workspace" />;
  }
  ...
  return (
    <AuthLayout eyebrow="Get started" title="Create your account">
      ...
      <form className="auth-form" onSubmit={handleSubmit} noValidate>
        <Field label="Username" htmlFor="username" hint="Minimum 3 characters" required>
          <TextInput
            id="username"
            name="username"
            type="text"
        ...
        <Field label="Password" htmlFor="password" required>
          ...
          {password.length > 0 && (
            <div className="auth-form__strength">
              <div className="auth-form__strength-header">
                <span className="auth-form__strength-label">Password Strength:</span>
                <span className={`auth-form__strength-value auth-form__strength-value--${strength.tone}`}>
                  {strength.label}
                </span>
              </div>
              <div className="auth-form__strength-bar">
                <div
                  className={`auth-form__strength-fill auth-form__strength-fill--${strength.tone}`}
