...
async function sendGmailMessage({ refreshToken, from, to, subject, text, attachment }) {
  ...
  const lines = [`From: ${safe(from)}`, `To: ${safe(to)}`, `Subject: ${safe(subject)}`, "MIME-Version: 1.0", `Content-Type: multipart/mixed; boundary="${boundary}"`, "", `--${boundary}`, "Content-Type: text/plain; charset=UTF-8", "Content-Transfer-Encoding: 8bit", "", String(text || "Please find my resume attached."), "", `--${boundary}`, `Content-Type: ${safe(attachment.contentType || "application/pdf")}; name="${safe(attachment.filename)}"`, "Content-Transfer-Encoding: base64", `Content-Disposition: attachment; filename="${safe(attachment.filename)}"`, ""];
