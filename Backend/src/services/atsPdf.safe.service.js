...
function generatePdfKitResume(resume = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (contactLine) doc.moveDown(0.25).font("Helvetica").fontSize(8.5).fillColor(muted).text(contactLine, { align: "center", lineGap: 1 });
    doc.moveDown(0.5).moveTo(48, doc.y).lineTo(564, doc.y).strokeColor(navy).lineWidth(1).stroke();
    doc.moveDown(0.45);

    const section = (title) => {
      if (doc.y > 720) doc.addPage();
      doc.font("Helvetica-Bold").fontSize(10.5).fillColor(navy).text(title.toUpperCase());
      doc.moveDown(0.15).moveTo(48, doc.y).lineTo(564, doc.y).strokeColor(rule).lineWidth(0.6).stroke();
      doc.moveDown(0.28);
    };
    ...
    if (Array.isArray(resume.experience) && resume.experience.length) {
      ...
      resume.experience.forEach((exp) => {
        doc.font("Helvetica-Bold").fontSize(10).fillColor(navy).text(exp.company || "", { continued: !!exp.dates });
        if (exp.dates) doc.font("Helvetica").fontSize(8.8).fillColor(muted).text(`  ${exp.dates}`, { align: "right" });
        if (exp.title) doc.font("Helvetica-Bold").fontSize(9).fillColor(muted).text(exp.title);
        if (exp.location) doc.font("Helvetica-Oblique").fontSize(8.5).fillColor(muted).text(exp.location);
        if (Array.isArray(exp.bullets)) exp.bullets.forEach((bullet) => addWrapped(doc, `• ${bullet}`, { x: 58, width: 506, lineGap: 1.5 }));
        doc.moveDown(0.35);
      });
    }
    ...
    if (Array.isArray(resume.education) && resume.education.length) {
      ...
      resume.education.forEach((edu) => {
        doc.font("Helvetica-Bold").fontSize(10).fillColor(navy).text(edu.degree || "", { continued: !!edu.institution });
        if (edu.institution) doc.font("Helvetica").fontSize(9).fillColor(muted).text(` — ${edu.institution}`);
        if (edu.dates) doc.font("Helvetica").fontSize(8.8).fillColor(muted).text(edu.dates);
        if (edu.details) addWrapped(doc, edu.details, { width: 516, lineGap: 1.5 });
        doc.moveDown(0.3);
      });
    }
    ...
    if (Array.isArray(resume.projects) && resume.projects.length) {
      ...
      resume.projects.forEach((project) => {
        doc.font("Helvetica-Bold").fontSize(10).fillColor(navy).text(project.name || "", { continued: !!project.role });
        if (project.role) doc.font("Helvetica").fontSize(9).fillColor(muted).text(` — ${project.role}`);
        if (project.dates) doc.font("Helvetica").fontSize(8.8).fillColor(muted).text(project.dates);
        if (Array.isArray(project.bullets)) project.bullets.forEach((bullet) => addWrapped(doc, `• ${bullet}`, { x: 58, width: 506, lineGap: 1.5 }));
        doc.moveDown(0.3);
      });
    }
...
async function generateAtsPdfBufferSafe(resume, cacheKey = null) {
  try {
    ...
    console.warn("ATS Chromium PDF unavailable; using PDFKit fallback:", error?.message || error);
