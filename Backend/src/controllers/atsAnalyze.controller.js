...
function buildTargetedOptimization(originalAnalysis, requestedSections = "all") {
  ...
  const targetedSuggestions = targets.flatMap((item) => {
    ...
    return [{ section: SECTION_LABELS[item.section], suggestion: `Improve the ${SECTION_LABELS[item.section]} section while preserving all original facts and information.`, reasoning: `${SECTION_LABELS[item.section]} scored ${item.score}/100 and is a priority for improvement.` }];
  });
...
async function atsAnalyzeController(req, res, next) {
  ...
  try {
    ...
    const atsReport = await atsReportModel.create({ user: userId, rawResumeText: resumeContent, resumeFileName: fileName, title: String(req.body.title || getDefaultTitle(fileName, analysisResult)).trim(), category, analysis: analysisResult, revisedResume: null });
    tokenReserved = false;
...
async function atsDownloadController(req, res, next) {
  ...
  try {
    ...
    if (!revisedResume) {
      ...
      tokenReserved = true;
      const optimized = await optimizeRevision({ originalText: report.rawResumeText, originalAnalysis: report.analysis, suggestions: report.analysis?.topSuggestions || [], sections: "all" });
