...
async function atsManualRevisionController(req, res, next) {
  try {
    ...
    if (!revisedResume || typeof revisedResume !== "object" || Array.isArray(revisedResume)) return res.status(400).json({ message: "A valid edited resume is required." });
    ...
    return res.status(200).json({ message: "Manual edits saved and PDF regenerated.", revisedResume, pdfReady: Boolean(pdfBuffer?.length) });
