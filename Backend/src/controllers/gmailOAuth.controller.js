...
async function startGmailOAuthController(req, res, next) {
  try {
    ...
    const state = jwt.sign(
      { purpose: "gmail-send", userId: String(user._id), returnTo: safeReturnTo(req.query.returnTo) },
      env.JWT_SECRET,
      { expiresIn: "10m" },
    );
...
async function gmailOAuthCallbackController(req, res) {
  ...
  try {
    ...
    if (oauthError) throw new Error(`Google authorization was cancelled or denied (${oauthError}).`);
    ...
    if (result.email !== accountEmail) {
      const error = new Error(`Connect the same Google account as your Resume Analyzer account (${accountEmail}).`);
      error.code = "GMAIL_ACCOUNT_MISMATCH";
      throw error;
    }
    ...
    return res.redirect(`${redirectBase}/gmail-connect-error?message=${message}`);
  }
}
...
async function gmailStatusController(req, res, next) {
  try {
    ...
    return res.status(200).json({
      connected: Boolean(user.gmailOAuthRefreshToken || user.gmailOAuthEmail),
