...
const {
  ...
  exchangeCode,
  encryptRefreshToken,
} = require("../services/gmailOAuth.service");
...
async function gmailOAuthCallbackController(req, res) {
  ...
  try {
    ...
    if (!code || !state)
      throw new Error("Missing Google OAuth authorization response.");
    const payload = jwt.verify(state, env.JWT_SECRET);
    if (payload.purpose !== "gmail-send" || !payload.userId)
    ...
    if (result.email !== accountEmail)
      throw new Error(
        `Connect the same Google account as your Resume Analyzer account (${accountEmail}).`,
      );
    ...
    await user.save();
    return res.redirect(
      `${redirectBase}${returnTo}${returnTo.includes("?") ? "&" : "?"}gmail=connected`,
    );
  } catch (error) {
    const message = encodeURIComponent(
      error.message || "Unable to connect Gmail.",
    );
    return res.redirect(
      `${redirectBase}${returnTo}${returnTo.includes("?") ? "&" : "?"}gmail=error&message=${message}`,
    );
  }
}
async function gmailStatusController(req, res, next) {
  try {
    const user = await userModel
      .findById(getUserId(req))
      .select("email gmailOAuthEmail gmailOAuthConnectedAt");
    if (!user)
      return res.status(404).json({ message: "User account not found." });
    return res
      .status(200)
      .json({
        connected: Boolean(user.gmailOAuthEmail),
        email: user.gmailOAuthEmail || null,
        accountEmail: user.email,
        connectedAt: user.gmailOAuthConnectedAt || null,
      });
  } catch (error) {
    next(error);
  }
