...
function getClientUrl() { return String(env.CLIENT_URL || "http://localhost:5173").split(",")[0].trim().replace(/\/$/, ""); }

async function stripeRequest(path, params = {}, method = "POST") {
  if (!env.STRIPE_SECRET_KEY) { const error = new Error("Stripe is not configured on the server"); error.statusCode = 503; throw error; }
  const options = { method, headers: { Authorization: `Bearer ${env.STRIPE_SECRET_KEY}` } };
  if (method !== "GET") { options.headers["Content-Type"] = "application/x-www-form-urlencoded"; options.body = new URLSearchParams(params); }
  const response = await fetch(`https://api.stripe.com/v1/${path}`, options);
  const data = await response.json();
  if (!response.ok) { const error = new Error(data?.error?.message || "Stripe request failed"); error.statusCode = response.status >= 400 && response.status < 500 ? 400 : 502; throw error; }
  return data;
}
...
async function creditCompletedCheckout(session) {
  ...
  const user = await userModel.findOneAndUpdate(
    ...
    { $set: { plan, lastStripeCheckoutSessionId: session.id }, $addToSet: { processedStripeCheckoutSessionIds: session.id }, $inc: { aiTokens: tokens } },
...
async function createCheckoutSessionController(req, res, next) {
  try {
    ...
    return res.status(200).json({ url: session.url, sessionId: session.id, plan: requestedPlan, planLabel: config.label, tokens: config.tokensPerPurchase, priceCents, currentTokens: Number(user.aiTokens) || 0 });
  } catch (err) { next(err); }
}
...
function verifyStripeSignature(rawBody, signatureHeader) {
  ...
  return signatures.some((signature) => { const a = Buffer.from(expected, "hex"); const b = Buffer.from(signature, "hex"); return a.length === b.length && crypto.timingSafeEqual(a, b); });
}
...
async function confirmCheckoutSessionController(req, res, next) {
  try {
    ...
    if (!session.metadata?.userId || String(session.metadata.userId) !== String(req.user.id)) return res.status(403).json({ message: "Checkout session does not belong to this user" });
    const result = await creditCompletedCheckout(session);
    const billing = await getBillingSnapshot(req.user.id);
    return res.status(200).json({ confirmed: session.payment_status === "paid", credited: result.credited || result.alreadyCredited === true, purchasedPlan: normalizePlan(session.metadata?.plan), purchasedTokens: Number(session.metadata?.tokens) || 0, plan: billing?.plan || "free", planLabel: billing?.planLabel || "Free", aiTokens: billing?.aiTokens || 0, tokensPerPurchase: billing?.tokensPerPurchase || 3000, planTokens: billing?.planTokens || 3000, tokenCosts: billing?.tokenCosts || {} });
  } catch (err) { next(err); }
}

async function getBillingStatusController(req, res, next) {
  try { const billing = await getBillingSnapshot(req.user.id); if (!billing) return res.status(404).json({ message: "User not found" }); return res.status(200).json(billing); } catch (err) { next(err); }
}
