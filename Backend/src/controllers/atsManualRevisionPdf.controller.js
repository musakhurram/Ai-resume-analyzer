...
function getRevisionPdfCacheKey(report) {
  const version = report.updatedAt ? new Date(report.updatedAt).getTime() : Date.now();
...
async function atsManualRevisionPdfController(req, res, next) {
  try {
    ...
    const report = await atsReportModel.findById(req.params.id).select("user revisedResume updatedAt");
