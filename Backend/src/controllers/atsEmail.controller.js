...
async function sendAtsEmailController(req, res, next) {
  try {
    ...
    const user = await userModel.findById(userId).select("email gmailOAuthRefreshToken gmailOAuthEmail");
    ...
    if (requestedSender !== accountEmail)
      return res.status(403).json({ message: "The sender email must match your logged-in account." });

    if (!user.gmailOAuthRefreshToken || String(user.gmailOAuthEmail || "").trim().toLowerCase() !== accountEmail) {
      return res.status(409).json({
        message: "Connect Gmail before sending. If Gmail was already connected, reconnect it to refresh the authorization.",
    ...
    if (attachment === "original") {
      ...
      if (!report.revisedResume)
        return res.status(409).json({ message: "Generate the AI-revised resume before sending the optimized PDF." });
    ...
    try {
      ...
      if (error?.code === "GMAIL_REFRESH_TOKEN_REVOKED" || error?.code === "GMAIL_TOKEN_INVALID" || error?.code === "GMAIL_REFRESH_TOKEN_MISSING") {
        ...
        await user.save();
        return res.status(409).json({
          message: "Your Gmail authorization is no longer valid. Please reconnect Gmail and try again.",
      ...
      if (error?.code === "GMAIL_SENDER_MISMATCH") {
        return res.status(409).json({
          message: "The connected Gmail account does not match your Resume Analyzer account. Reconnect Gmail using the same Google account you use here.",
