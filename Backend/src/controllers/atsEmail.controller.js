...
async function sendAtsEmailController(req, res, next) {
  try {
    ...
    const user = await userModel
      .findById(userId)
      .select(
        "email gmailOAuthEmail gmailOAuthConnectedAt +gmailOAuthRefreshToken",
      );
    if (!user)
    ...
    const connectedEmail = String(user.gmailOAuthEmail || "")
      .trim()
      .toLowerCase();
    if (!user.gmailOAuthRefreshToken || connectedEmail !== accountEmail) {
      return res
        ...
        .json({
          message:
            "Connect Gmail before sending. Use the same Google account as your Resume Analyzer account.",
    ...
    try {
      ...
      if (
        ...
        return res
          ...
          .json({
            message:
              "Your Gmail authorization is no longer valid. Please reconnect Gmail and try again.",
      ...
      if (error?.code === "GMAIL_SENDER_MISMATCH") {
        return res
          ...
          .json({
            message:
              "The connected Gmail account does not match your Resume Analyzer account. Reconnect Gmail using the same Google account you use here.",
      ...
      if (error?.code === 401 || error?.response?.status === 401) {
        return res
          ...
          .json({
            message:
              "Gmail rejected the current authorization. Please reconnect Gmail and try again.",
            code: "GMAIL_RECONNECT_REQUIRED",
          });
      }
      ...
      if (error?.code === 403 || error?.response?.status === 403) {
        ...
        return res
          ...
          .json({
            message:
              googleMessage ||
              "Gmail rejected the request. Check that Gmail sending permission is enabled for this Google account.",
